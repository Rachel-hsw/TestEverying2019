...
public abstract class BaseApi
{
  ...
  public static void handleDataToListener(Intent paramIntent, IUiListener paramIUiListener)
  {
    ...
    do
    {
      ...
      {
        return;
        str1 = paramIntent.getStringExtra("key_action");
        if (!"action_login".equals(str1))
          continue;
        int i = paramIntent.getIntExtra("key_error_code", 0);
        if (i == 0)
        {
          String str5 = paramIntent.getStringExtra("key_response");
          if (str5 != null)
            try
            {
              paramIUiListener.onComplete(Util.parseJson(str5));
              return;
            }
            catch (JSONException localJSONException2)
            {
              paramIUiListener.onError(new UiError(-4, "服务器返回数据格式有误!", str5));
              f.b(f.d, "OpenUi, onActivityResult, json error", localJSONException2);
              return;
            }
        ...
        f.e(f.d, "OpenUi, onActivityResult, onError = " + i + "");
      ...
      while (!"action_share".equals(str1));
      str2 = paramIntent.getStringExtra("result");
      str3 = paramIntent.getStringExtra("response");
    ...
    while (!"complete".equals(str2));
    if (str3 == null);
    for (String str4 = "{\"ret\": 0}"; ; str4 = str3)
      try
      {
        paramIUiListener.onComplete(new JSONObject(str4));
        return;
      }
  ...
  {
    StringBuilder localStringBuilder = new StringBuilder();
    localStringBuilder.append("http://qzs.qq.com/open/mobile/login/qzsjump.html?");
    localStringBuilder.append(Util.encodeUrl(paramBundle));
    new TDialog(paramActivity, "", localStringBuilder.toString(), null, this.mToken).show();
  }
    ...
    implements IRequestListener
  {
    ...
    public TempRequestListener(IUiListener arg2)
    {
      Object localObject;
      this.mListener = localObject;
      this.mHandler = new BaseApi.TempRequestListener.1(this, Global.getContext().getMainLooper(), BaseApi.this);
    }
