  ...
  implements View.OnTouchListener
{
  ...
  private float getEdgeValue(float paramFloat1, float paramFloat2, float paramFloat3, float paramFloat4)
  {
    ...
    float f5;
    if (f3 < 0.0F)
      f5 = -this.mEdgeInterpolator.getInterpolation(-f3);
    while (true)
    {
  ...
  private static class ClampedScroller
  {
    ...
    private float getValueAt(long paramLong)
    {
      ...
      if ((this.mStopTime < 0L) || (paramLong < this.mStopTime))
        return 0.5F * AutoScrollHelper.access$900((float)(paramLong - this.mStartTime) / this.mRampUpDuration, 0.0F, 1.0F);
      long l = paramLong - this.mStopTime;
      return 1.0F - this.mStopValue + this.mStopValue * AutoScrollHelper.access$900((float)l / this.mEffectiveRampDown, 0.0F, 1.0F);
    }
