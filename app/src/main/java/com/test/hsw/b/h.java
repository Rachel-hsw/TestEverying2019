package b;

import java.io.IOException;
import java.io.OutputStream;
import java.nio.charset.Charset;

public abstract interface h extends aa
{
  public abstract long a(ab paramab)
    throws IOException;

  public abstract h b(j paramj)
    throws IOException;

  public abstract h b(String paramString)
    throws IOException;

  public abstract h b(String paramString, Charset paramCharset)
    throws IOException;

  public abstract h b(byte[] paramArrayOfByte)
    throws IOException;

  public abstract h c(byte[] paramArrayOfByte, int paramInt1, int paramInt2)
    throws IOException;

  public abstract e d();

  public abstract OutputStream e();

  public abstract h g(int paramInt)
    throws IOException;

  public abstract h h(int paramInt)
    throws IOException;

  public abstract h i(int paramInt)
    throws IOException;

  public abstract h j(int paramInt)
    throws IOException;

  public abstract h j(long paramLong)
    throws IOException;

  public abstract h k(int paramInt)
    throws IOException;

  public abstract h k(long paramLong)
    throws IOException;

  public abstract h y()
    throws IOException;
}

/* Location:           C:\Androidnixiangzhushou\zikao_classes_dex2jar.jar
 * Qualified Name:     b.h
 * JD-Core Version:    0.6.0
 */